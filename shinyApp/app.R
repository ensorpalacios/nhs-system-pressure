...
ui <- page_navbar(
  ...
  tags$head(
    ...
    /* Target ONLY the cards inside the plot grid, not the header card */
    ...
    /* Force ggiraph SVG to scale fluidly like a standard ggplot */
    .custom-card-body .html-widget svg {
  ...
  nav_panel(
    ...
    card(
      ...
      card_body(
        ...
        # Snug padding (Top/Bottom, Left/Right)
...
server <- function(input, output) {
  ...
  # Get data
  risk_d <- as.data.table(model_out)[type == "risk_d" &
                                       .model == model, .(site, index, risk_day)]
  risk_ws_close <- as.data.table(model_out)[type == "risk_ws" &
                                              .model == model & week_split == "close", .(site, risk_ws)]
  risk_ws_far <- as.data.table(model_out)[type == "risk_ws" &
                                            .model == model & week_split == "far", .(site, risk_ws)]
  risk_w <- as.data.table(model_out)[type == "risk_w" &
                                       .model == model, .(site, risk_w)]
  fc <- as.data.table(model_out)[type == "forecast" &
                                   .model == model]
  thr <- as.data.table(model_out)[type == "risk_w" &
                                    .model == model, .(site, thr)]
